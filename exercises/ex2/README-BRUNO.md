...
After completing these steps you will have created and stored a prompt template, and evaluated it wiht a set of metrics. 
...
4. You should receive a response messages that the prompt was created successfully. A script copies the prompt template id from the response body and stores it in an environmant variable for later use.
...
2. Not that the request uses the environmant variable promptTamplateID in the request URL.
3. Execute the requeste by clicking the left arrow.
4. You should see your stored prompt template in the.

## Exercise 2.2 Uplaod Evaluation Data Set
...
4. Execute the request to upload the data set to the objetc sotre via the dataset API.
...
4. You should receive a response messages that the artifact was acknowledged. A script copies the artifact id from the response body and stores it in an environmant variable for later use.
...
Now that we have successfully prepared a prompt template and an artifact with our evaluation data set, we can configure and run an evakuation with the AI Core evaluation service. 
